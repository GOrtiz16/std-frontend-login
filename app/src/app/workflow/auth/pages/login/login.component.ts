...
import { CommitInfoService } from 'src/app/shared/services/commit-info.service';
...
export class LoginComponent {
  ...
  ngOnInit() {
    this.initCounter();
    this.commitInfoService.getCommitInfo().subscribe((info) => (this.version = `v${info.appVersion}`));
  ...
  // Flujo Temporal
  userNotFoundFlow(error: any): void {
    const errors = error.error?.errors;
    const hasError1019 = !!errors?.find((e: any) => e.code == '1019');

    if (!hasError1019 || error.status !== 404) {
      return;
    }

    let retry = +(sessionStorage.getItem('r') || 0);
    retry = retry + 1;
    sessionStorage.setItem('r', `${retry}`);
  ...
  onInputUsernameBlur(event: any): void {
    this.loginPresenter.form.controls['username'].setValue(event.target.value.trim());
  }
